MODULE_PARAMS="name=org.macports.MacPorts"
MODULE_PARAMS="$MODULE_PARAMS state=present"
#MODULE_PARAMS="$MODULE_PARAMS required_version=0.2.2.0.0.0.0.0.0x"
MODULE_PARAMS="$MODULE_PARAMS dest=/tmp/macports.pkg"
MODULE_PARAMS="$MODULE_PARAMS url=https://distfiles.macports.org/MacPorts/MacPorts-2.2.0-10.8-MountainLion.pkg"
#echo $MODULE_PARAMS
~/workspace/32degrees/ansible/hacking/test-module -m library/packaging/mac_pkg -a "$MODULE_PARAMS"
